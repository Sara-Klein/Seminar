...
class Multilevel():
    ...
    '''
        self.network = PolicyNet(a,b) für richtige dimensionen a,b
        theta_0 braucht man nicht
    '''
    ...
    def sample_env(self, theta, t, state): 
        ########################## input action fehlt ############ self.network übergeben - mit netzwerk arbeiten statt mit theta!
    ...
    def multi_level_step(self):
        ...
        self.theta = self.theta_0.clone().detach()  ########## braucht man nicht?
        print(self.theta) 
        '''
            mit optimzer arbeiten:
            optimizer = optim.SGD(self.network.parameters())
            optimizer.param_groups[0]['lr'] = 0.001 (oder was auch immer)
        '''
            ...
            for k in range(1,m_n+1):
                ...
                for l in range(1,N_nk+1):
                    ...
                    for t in range(TIME-1):
                        ...
                        rew_to_go = 0
                        for u in range(M**k):
                            _, rew_to_go = self.sample_env(self.theta, state=state, action=action) #### self.network übergeben - mit netzwerk arbeiten statt mit theta
                        ...
                        if k ==1:
                            ...
                            for u in range(M**(k-1)):
                                _,rew_to_go2 = self.sample_env(self.theta, t, state=state, action=action) #### self.network übergeben - mit netzwerk arbeiten statt mit theta
                                # rew = np.array(t_to_go).reshape(int((len(t_to_go))/2),2)
                                # for tup in rew[0:]:
                    ...
                    with torch.set_grad_enabled(True):
                        loss = ? ..... hier alles was mit dem netzwerk zu tun hat in der Berechnung und was abgeleitet werden soll!, der rest kann vorher berechnet werden!
                        loss.backward()
                        optimizer.step()
                    scheduler.step()
                    '''
