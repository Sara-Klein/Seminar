...
class Multilevel():
    def __init__(self, env=Connect4Env()) -> None:
        self.env = env
        self.theta_0 = np.ones(len(self.env.observation_space))
        self.m = M
        self.sigma_0 = 1
        self.eps_0 = 1
        self.rho = (1/2) * (1+R)
    ...
    def sample_env(self, theta, t, state):
        ...
        # Ich kann auc versuchen Offline Reinforcement Learning zu etablieren
        rewards = 0
        if action == None:
            trajectory = [state]
        ...
        for n in range(1,5): # 5 frei gewählt, um schnelle Resultate zu sehen
            ...
            for k in range(1,m_n+1):
                ...
                for l in range(1,N_nk+1):
                    traject = self.sample(self.theta,t=0, state=0, action=None)
                    acts = list()
                    Q_k = list()
                    Q_k2 = list()
                    for t in range(TIME-1):
                        ...
                        rew_to_go = 0
                        for u in range(M**k):
                            t_to_go = self.sample(self.theta, t, state=state, action=action)
            ...
            plt.show()
            print(np.sum(L_m))
            self.theta = self.theta + gamma_n * L_m
        return self.theta
